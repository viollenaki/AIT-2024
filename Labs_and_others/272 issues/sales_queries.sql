-- ============================================
-- sales_queries.sql (SQLite)
-- Take-Home: Sales & Customer Analysis
-- ============================================

-- 1) create table
-- order_id can repeat (one order may have multiple products),
-- so I use "id" as unique row identifier.
...
CREATE TABLE sales (
  ...
  category TEXT,                  -- can be NULL, later we can treat as 'Unknown'
  price REAL NOT NULL,
...
-- 2) insert data
-- If you already import from CSV, you can skip INSERTs.
-- Example import idea:
-- .mode csv
...
-- order_date is text but in YYYY-MM-DD, so strftime works fine
-- ============================================
SELECT
...
-- ============================================
-- Query 6: customers with total spending above average customer spending
-- (average is calculated over customers totals, not over rows)
-- ============================================
SELECT
  customer_id,
